...
Bab ini menyajikan hasil implementasi sistem ``GOPOS (\textit{Go-Lang POS Assistant})'' sebagai validasi praktis terhadap temuan tinjauan literatur sistematis yang telah dibahas pada Bab II. Implementasi sistem \textit{chatbot} cerdas ini telah berhasil memenuhi kebutuhan pengguna dalam mengakses informasi layanan Pos Indonesia secara efektif dan efisien. Dengan fitur utama seperti asisten virtual berbasis Gemini AI --- yang oleh tinjauan literatur terbukti lebih efektif dibandingkan pendekatan \textit{rule-based} --- serta fitur cek tarif pengiriman (Cek Ongkir), sistem ini mampu berjalan sesuai dengan perancangan yang telah ditetapkan. Pengguna dapat berinteraksi dengan sistem secara optimal melalui antarmuka web yang responsif serta WhatsApp Gateway.
...
Halaman \textit{dashboard} admin dirancang sebagai pusat kendali operasional dan pemantauan interaksi \textit{omni-channel} yang terintegrasi melalui sistem navigasi tab, yang mencakup modul Manajemen User, Riwayat Chat (Web), dan WhatsApp Chat. Pada modul Manajemen User, administrator dapat memantau metrik statistik pengguna secara \textit{real-time} serta melakukan kontrol akses dan operasi CRUD (\textit{Create, Read, Update, Delete}) untuk menjaga validitas serta keamanan data akun. Sistem ini juga dilengkapi dengan indikator status \textit{Live} yang memastikan sinkronisasi data antara antarmuka admin dengan basis data berjalan secara kontinu.

Sementara itu, modul Riwayat Chat dan WhatsApp Chat berfungsi sebagai instrumen transparansi untuk mendokumentasikan setiap interaksi antara pengguna dengan Gemini AI, baik melalui antarmuka web maupun platform pesan instan. Melalui fitur peninjauan detail percakapan, administrator dapat melakukan audit terhadap kualitas respon bot serta menganalisis tren kebutuhan pelanggan berdasarkan log percakapan yang tersimpan secara sistematis. Keseluruhan antarmuka ini didukung oleh fitur pencarian dan paginasi yang responsif, memastikan efisiensi dan akurasi dalam pengelolaan pusat data layanan pada PT Pos Indonesia.
